...
const HEADING_GRADIENT_RE = /bg-gradient-to-/
const HEADING_VAR_FROM_RE = /from-\[var\(--hd-/
...
function transform(content, className) {
  ...
  if (HEADING_GRADIENT_RE.test(out) && !HEADING_VAR_FROM_RE.test(out)) {
...
function rewriteOneFile(filePath) {
  ...
  // 例: heading で始まるもの全部
  const genericMatcher = /\[\s*(['"])(heading\d[0-9A-Za-z-]*)\1\s*,\s*`([\s\S]*?)`\s*\],?/g

  out = out.replace(genericMatcher, (m, quote, name, body) => {
    // すでにピンポイントで処理したものはスキップ（2重変換防止）
    if (TARGET_CLASS_NAMES.includes(name))
      return m
    const newBody = transform(body, name)
    if (newBody !== body)
      changed = true
